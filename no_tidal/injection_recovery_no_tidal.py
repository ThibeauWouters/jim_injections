...
from jimgw.single_event.waveform import RippleTaylorF2_NoTidal, RippleIMRPhenomD_NRTidalv2_NoTidal
...
def body(args):
    ...
    # Fetch waveform used
    supported_waveforms = ["TaylorF2_NoTidal", "NRTidalv2_NoTidal", "IMRPhenomD_NRTidalv2_NoTidal", "NRTv2_NoTidal"]
    ...
    if args.waveform_approximant == "TaylorF2_NoTidal":
        ripple_waveform_fn = RippleTaylorF2_NoTidal
    elif args.waveform_approximant == "IMRPhenomD_NRTidalv2_NoTidal":
        ripple_waveform_fn = RippleIMRPhenomD_NRTidalv2_NoTidal
    else:
        raise ValueError(
    ...
    if args.which_distance_prior == "uniform":
        ...
        dL_prior = PowerLaw(prior_low[4], prior_high[4], 2.0, naming=['d_L'])
    
    tc_prior = Uniform(prior_low[5], prior_high[5], naming=['t_c'])
    phic_prior = Uniform(prior_low[6], prior_high[6], naming=['phase_c'])
    cos_iota_prior = Uniform(prior_low[7], prior_high[7], naming=["cos_iota"],
                             transforms={
        "cos_iota": (
            "iota",
    ...
    sin_dec_prior = Uniform(prior_low[10], prior_high[10], naming=["sin_dec"],
                            transforms={
        "sin_dec": (
            "dec",
    ...
    if args.smart_initial_guess:
        ...
        utils.plot_chains(initial_guess, "initial_guess",
                          outdir, truths=truths, n_dim=len(prior_list))
    else:
    ...
    utils.plot_chains(chains, "chains_production", outdir, truths=truths, n_dim=len(prior_list))
    ...
    # TODO debug this
    # utils.plot_chains(chains, "chains_NF", outdir, truths=truths, n_dim=len(prior_list))
