...
def body(args):
    ...
    while network_snr < args.SNR_threshold:
        ...
        # TODO have to check whether this is valid
        config["t_c"] = 0.0
        ...
        for idx, ifo in enumerate(ifos):
            name = ifo.name
            print(name)
            filename = outdir + f"{name}_data_injection.npz"
            bilby_data = np.load(filename)
            freqs = jnp.array(bilby_data["frequency"])
            data = jnp.array(bilby_data["data"])
            ifo.frequencies = freqs
            ifo.data = data
            
            # Also load the PSD
            psd_file = psd_files[idx]
            psd = ifo.load_psd(freqs, psd_file)
            ifo.psd = psd
            
        print("Signal loaded!")
        
        ### Now, we load the data and override the injection
        
        # Generating the geocenter waveform
        print("Constructing reference waveform")
        h_sky = waveform(freqs, true_param)
        print("Constructing reference waveform DONE")
        # Compute the SNR
