"""
Note: this is a notebook where I try to implement the TF2 warmup
"""
import psutil
p = psutil.Process()
p.cpu_affinity([0])
...
def body(args):
    ...
    while network_snr < args.SNR_threshold:
        ...
        waveform_training = RippleTaylorF2(f_ref = config["fref"])
        waveform_production = RippleIMRPhenomD_NRTidalv2(f_ref = config["fref"])
        ...
        h_sky = waveform_production(freqs, true_param)
    ...
    likelihood_training = HeterodynedTransientLikelihoodFD(
        ifos,
        prior=complete_prior,
        bounds=bounds,
        n_bins = args.relative_binning_binsize,
        waveform=waveform_training,
    ...
    likelihood_production = HeterodynedTransientLikelihoodFD(
        ifos,
        prior=complete_prior,
        bounds=bounds,
        n_bins = args.relative_binning_binsize,
        waveform=waveform_production,
        trigger_time=config["trigger_time"],
        duration=config["duration"],
        post_trigger_duration=config["post_trigger_duration"],
        ref_params=ref_params # put the reference waveform of the relative binning at the true parameters
    ...
    # end_likelihood = time.time()
    # print(f"Time taken to initialize the likelihood: {end_likelihood - start_likelihood}s, {(end_likelihood - start_likelihood) / 60} min")
