...
def get_parser(**kwargs):
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="Whether to load and redo an existing injection.",
    )
    parser.add_argument(
        "--N",
        type=str,
        default="",
        help="Number (or generically, an identifier) of this injection, used to locate the output directory. If an empty string is passed (default), we generate a new injection.",
    )
    parser.add_argument(
        "--SNR_THRESHOLD",
        type=float,
    ...
    # TODO has to be implemented here:
    parser.add_argument(
        "--autotune_local_sampler",
        type=bool,
        default=True,
        help="TODO has to be implemented! Specify whether to use autotuning for the local sampler.",
    )
    return parser
...
def body(args):
    ...
    while network_snr < args.SNR_THRESHOLD:
        ...
        # Get outdir (including the injection identifier at the end) and PRNGKey
